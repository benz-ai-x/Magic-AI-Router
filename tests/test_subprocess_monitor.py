...
class TestProperties(unittest.TestCase):
    ...
    def test_cmd_str_set_by_start_process(self):
        m = _FakeMonitor()
        with patch("shared.subprocess_monitor.subprocess.Popen") as mock_popen:
...
class TestStartProcess(unittest.TestCase):
    def test_popen_failure_sets_error(self):
        m = _FakeMonitor()
        with patch("shared.subprocess_monitor.subprocess.Popen", side_effect=OSError("no such file")):
    ...
    def test_successful_start_sets_starting(self):
        ...
        with patch("shared.subprocess_monitor.subprocess.Popen", return_value=mock_proc):
