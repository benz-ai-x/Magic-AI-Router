...
class SshSession:
    ...
    Args:
        ...
        socks5_port_fn: () -> int|None。None = 纯 -L（无 -D）。
        probe_port_fn: () -> int|None。健康检查的就绪探测口；缺省 =
            socks5 端口，纯 -L 模式取 spawn 副本第一条 -L 的本地端口。
    """

    def __init__(self, log_sink, identity_fn, password_fn,
                 spawn_fn=None, socks5_port_fn=lambda: None,
                 probe_port_fn=None):
    ...
    @property
    def probe_port(self):
        if self._probe_port_fn is not None:
            return self._probe_port_fn()
        socks5 = self._socks5_port_fn()
        if socks5 is not None:
            return socks5
        tunnel = self._spawn_fn() or {}
    ...
    def _start_now(self):
        ...
        self.monitor.start(tunnel, self._socks5_port_fn(), password)
